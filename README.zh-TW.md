...
- **兩種檢視模式 + Agent 面板** — Office（像素風）、Dashboard（卡片式），加上可拖曳的 Agent 清單覆蓋面板
- **區域網路共享** — 查看同一網路上其他機器的 agents
- **狀態歷史** — 點擊任意 agent 查看近期活動歷史
- **視窗控制** — 置頂、系統匣、最小化至系統匣
- **最小化模式** — 使用 `-min` 參數啟動時隱藏於系統匣

## 系統匣

應用程式會自動建立系統匣圖示。右鍵點擊系統匣圖示可選擇：
- **Show Window** — 顯示視窗
- **Exit** — 結束應用程式

### 最小化模式（`-min`）

使用 `-min` 參數啟動，應用程式會隱藏在系統匣中，不顯示視窗：
...
此模式下：
- 啟動時不會建立視窗 — 應用程式在系統匣中靜默運行
- 點擊系統匣選單的 "Show Window" 開啟視窗
- 關閉視窗後回到系統匣，不會結束程式
- 使用系統匣選單的 "Exit" 完全退出
...
**List** 面板是一個精簡的可拖曳/可調整大小側邊面板，覆蓋在 Office 檢視上，按專案分組顯示所有 agents。可在 Office 模式下從底部工具列切換。

在所有檢視中，**點擊 agent** 可開啟彈出視窗，顯示最近 10 筆狀態事件（連續去重）。
